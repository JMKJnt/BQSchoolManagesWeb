...
function insertClassInfo() {
	...
	var jsonstr = dataHelper.setJson(null, "className", $("#className").val()); //班级名称
	...
	jsonstr = dataHelper.setJson(jsonstr, "classCreate", $.cookie("userName")); //创建人为当前用户$.cookie("userName")
	jsonstr = dataHelper.setJson(jsonstr, "studentList", getStudentList());
	jsonParam.jsonStr = jsonstr;
	console.log(jsonParam);
	classInfoServer.insertClassInfo(jsonParam, callback_insertClassInfo);
};
/**
 * 功能：新增班级信息回调
 * 创建人：liql
 ...
 * @param {Object} data
 */
function callback_insertClassInfo(data) {
	console.log(data);
	if(data == undefined || data.rspCode == null || data.rspCode != "000") {
		controlsHelper.alert(data.rspDesc);
	} else {
		controlsHelper.alert("添加班级信息成功", "20%");
		location.href = "classList.html";
	}
};
/*
