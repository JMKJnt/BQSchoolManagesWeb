...
function callback_getactiveCateData(data) {
	...
	if(data.rspCode == null || data.rspCode == undefined || data.rspCode != '000') {
		...
		$.each(data.lists, function(index, entry) {
			$("#activeCate").append("<label for='level1'><input type='radio' name='optionsRadios' value='" + entry.data_code + "' >" + entry.data_name + "&nbsp;&nbsp </label>");
		});
...
function insertActive() {
	...
	// console.log($("#periodSelect_id").val())
	var jsonParam = {
		"code": "10002",
		"version": "1.0",
		"jsonStr": {}
	};
	var jsonstr = dataHelper.setJson(null, "activeCate", $("#activeCate").find("input[type=radio][checked=checked]").attr("value")); //活动类型
	...
	activeClassNum = activeClassNum.length > 0 && activeClassNum.substr(activeClassNum.length - 1, 1) == "," ? activeClassNum.substr(0, activeClassNum.length - 1) : activeClassNum;
	...
	jsonstr = dataHelper.setJson(jsonstr, "activeCreate", "admin"); //$.cookie("userName")
	...
	jsonstr = dataHelper.setJson(jsonstr, "activityHase",$("#periodSelect_id").val());
	
	jsonParam.jsonStr = jsonstr;
	console.log(jsonParam);
	activeInfoServer.insertActive(jsonParam, callback_insertActive);
};
 ...
 * 创建时间：2018-9-12
 */
function callback_insertActive(data) {
	console.log(data);
