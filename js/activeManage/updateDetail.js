...
$(document).ready(function() {
	...
	detailId = dataHelper.QueryString("detailId");
	//保存新增发券
 ...
 * 创建时间：2018-9-17
 */
function getDetailInfoById() {
	var jsonParam = {
		"code": "10002",
		"version": "1.0",
		"jsonStr": {}
	};
	jsonParam.jsonStr = dataHelper.setJson(null, "detailId", detailId); //班级id
	console.log(jsonParam);
	activeInfoServer.getDetailInfoById(jsonParam, callback_getDetailInfoById);
}
/*
 * 功能：获取活动信息回调
 * 创建人：liql
 * 创建时间：20148-9-17
 */
function callback_getDetailInfoById(data) {
	console.log(data);
	if(data == undefined || data.rspCode != "000" || data.rspCode == null) {
		controlsHelper.alert("获取明细信息失败：" + data.rspDesc);
	} else {
		if(data.detailBanner != "") {
			$(".imgName1").attr("src", data.detailBanner);
			$("#imgName1").attr("src", data.detailBanner);
			$("#imgName1").attr("data-filepath", data.detailBanner);
		}

		$("#detailTheme").val(data.detailTheme);
		$("#detailDesc").val(data.detailDesc);
		$("#detailStartTime").val(data.detailStartTime.substr(0, 10));
		$("#detailEndTime").val(data.detailEndTime.substr(0, 10));
		$("#detailZbcode").val(data.detailZbcode);
		$("#detailTeacher").find('li[value='+data.detailTeacherId+']').click();

		if(data.detailIsPublic == "1") {
			$("#detailIsPublic").attr("checked", "checked");
			$("#detailIsPublic").prop("checked", true);
		} else {
			$("#detailIsPublic").removeAttr("checked");
			$("#detailIsPublic").prop("checked", false);
		}
		...
		$("#detailCate").find("input[type=radio]").removeAttr('checked');
		$("#detailCate").find("input[type=radio]").prop("checled", false);
		$("#detailCate").find("input[type=radio][value=" + data.detailCate + "]").attr("checked", "checked");
		$("#detailCate").find("input[type=radio][value=" + data.detailCate + "]").prop("checked", true);
...
function updateDetail() {
	...
	var jsonstr = dataHelper.setJson(null, "detailId", detailId); //活动类型
	jsonstr = dataHelper.setJson(jsonstr, "detailCate", $("#detailCate").find("input[type=radio][checked=checked]").attr("value")); //活动类型
	jsonstr = dataHelper.setJson(jsonstr, "detailBanner", $("#imgName1").attr("src")); //活动banner
	...
	jsonstr = dataHelper.setJson(jsonstr, "updateName", dataHelper.isEmpty($.cookie("userName")) ? 'admin' : $.cookie("userName"));
	jsonstr = dataHelper.setJson(jsonstr, "detailZbcode", $("#detailZbcode").val());
	jsonParam.jsonStr = jsonstr;
	console.log(jsonParam);
	activeInfoServer.updateDetail(jsonParam, callback_updateDetail);
};
 ...
 * 创建时间：2018-9-12
 */
function callback_updateDetail(data) {
	console.log(data);
