...
cookies = decodeURIComponent(getCookie(cookies, 'BusinessUserCookie'));
	...
	$(".sidebar-menu").on("click", "a", function() {
		...
		if(tabId != 'undefined' && tabId != 'null' && tabId != '' && tabId != undefined && tabId != null) {
			...
			if(loadSign) {
				...
				if(ifmFullUrl.indexOf('$branchID$') >= 0 && jQuery.parseJSON(cookies)!=undefined && jQuery.parseJSON(cookies)!=null) {
					ifmFullUrl = ifmFullUrl.replace('$branchID$',jQuery.parseJSON(cookies).branchId);
					console.log(ifmFullUrl);
				}
